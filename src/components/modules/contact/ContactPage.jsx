          ...
          >
            <div className="space-y-4">
              ...
              <p className="text-sm font-medium text-slate-400 leading-relaxed max-w-sm">
                Have an enterprise project or open-source engineering loop to
                execute? Feel free to reach out directly via secure network
                endpoints.
              </p>
            </div>
            ...
            <div className="space-y-4 my-2">
              {/* NODE 1: EMAIL TERMINAL (CLICK TO COPY MECHANICS) */}
              <div
                onClick={() =>
                  handleCopyToClipboard("antodasahir@gmail.com", "email")
                }
                className="flex items-center justify-between gap-4 p-4 rounded-2xl border border-slate-800/80 bg-slate-950/40 backdrop-blur-sm shadow-md group hover:border-[#326fd14d] hover:bg-[#0e182d]/20 transition-all duration-300 cursor-pointer relative select-none"
              >
                <div className="flex items-center gap-4 truncate">
                  ...
                  <div className="truncate">
                    ...
                    <span className="text-sm sm:text-base font-bold text-white group-hover:text-[#326ed1] transition-colors truncate">
                      antodasahir@gmail.com
                    </span>
                  </div>
                </div>

                {/* CYBER TOAST FEEDBACK NOTIFICATION */}
                <span
                  className={`text-[10px] font-mono font-bold px-2 py-1 rounded-md transition-all duration-300 border ${
                    copiedState.email
                      ? "bg-emerald-950/80 text-emerald-400 border-emerald-800/60 scale-100 opacity-100"
                      : "bg-slate-900 text-slate-500 border-slate-800 opacity-0 group-hover:opacity-100 scale-95"
                  }`}
                >
              ...
              {/* NODE 2: CELL INBOUND ENDPOINT (CLICK TO COPY MECHANICS) */}
              <div
                onClick={() => handleCopyToClipboard("+8801830509696", "phone")}
                className="flex items-center justify-between gap-4 p-4 rounded-2xl border border-slate-800/80 bg-slate-950/40 backdrop-blur-sm shadow-md group hover:border-[#326fd14d] hover:bg-[#0e182d]/20 transition-all duration-300 cursor-pointer relative select-none"
              >
          ...
          >
            ...
            >
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 pl-1 block">
                      Full Identity
                    </label>
                    <input
                      type="text"
              ...
              >
                ...
                <HiPaperAirplane
                  className={`text-sm ${isSending ? "animate-pulse" : ""}`}
                />
              </button>
            </form>
          </div>
